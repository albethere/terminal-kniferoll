...
# ══════════════════════════════════════════════════════════════════════════════
# ZSCALER — WRITE ENV FILE
# ══════════════════════════════════════════════════════════════════════════════
# Writes $env:USERPROFILE\.config\terminal-kniferoll\zscaler-env.ps1 with
# Windows-specific detection paths. No $OSTYPE branching at runtime.
# All env vars derived from $Script:ZscPem. Called unconditionally on every run.

function Write-ZscalerEnvFile {
    $envDir  = "$env:USERPROFILE\.config\terminal-kniferoll"
    $envFile = "$envDir\zscaler-env.ps1"
...
foreach ($p in @(
    ...
    if ((Test-Path $p) -and (Get-Item $p -ErrorAction SilentlyContinue).Length -gt 0) {
        $Script:ZscPem = $p; break
    }
}

if ($Script:ZscPem) {
    $env:ZSC_PEM             = $Script:ZscPem
    $env:CURL_CA_BUNDLE      = $Script:ZscPem
    $env:SSL_CERT_FILE       = $Script:ZscPem
    $env:REQUESTS_CA_BUNDLE  = $Script:ZscPem
    $env:NODE_EXTRA_CA_CERTS = $Script:ZscPem
    $env:GIT_SSL_CAINFO      = $Script:ZscPem
    $env:AWS_CA_BUNDLE       = $Script:ZscPem
    $env:PIP_CERT            = $Script:ZscPem
}
'@
    $header = "# Auto-generated by terminal-kniferoll installer. DO NOT EDIT.`n# OS: Windows  Generated: $ts UTC`n`n"
    $content = $header + $body
    $bom = New-Object System.Text.UTF8Encoding($true)
    [System.IO.File]::WriteAllText($envFile, $content, $bom)
    Write-OK "Zscaler env file written: $envFile"
}
...
function Show-ZscalerStatus {
    Write-Host '=== terminal-kniferoll: Zscaler Status ===' -ForegroundColor Cyan
    Write-Host "OS: Windows  $(Get-Date -Format 'yyyy-MM-dd')" -ForegroundColor DarkCyan
    Write-Host ''
    ...
    $resolved = $null
    Write-Host 'Detection paths:' -ForegroundColor Cyan
    foreach ($p in $detectionPaths) {
        if ((Test-Path $p) -and (Get-Item $p -ErrorAction SilentlyContinue).Length -gt 0) {
    ...
    if ($resolved) {
        Write-Host "ZSC_PEM: $resolved" -ForegroundColor Green
    } else {
        Write-Host 'ZSC_PEM: (not found -- non-managed device)' -ForegroundColor Yellow
    }
    Write-Host ''

    $envVars = 'ZSC_PEM','CURL_CA_BUNDLE','SSL_CERT_FILE','REQUESTS_CA_BUNDLE',
    ...
    Write-Host 'TLS smoke test (npmjs.org)...' -ForegroundColor Cyan
    $tlsOk = $false
    try {
        $r = Invoke-WebRequest -UseBasicParsing -Uri 'https://registry.npmjs.org/' `
                -TimeoutSec 10 -ErrorAction Stop
        if ($r.StatusCode -lt 400) {
            Write-Host '  [PASS] TLS connectivity OK' -ForegroundColor Green
            $tlsOk = $true
        }
    } catch {
        Write-Host "  [FAIL] TLS test: $($_.Exception.Message)" -ForegroundColor Red
    }
    Write-Host ''

    $envFile = "$env:USERPROFILE\.config\terminal-kniferoll\zscaler-env.ps1"
    if (Test-Path $envFile) {
        Write-Host "Env file: $envFile [EXISTS]" -ForegroundColor Green
    } else {
        Write-Host "Env file: $envFile [MISSING -- run installer to generate]" -ForegroundColor Yellow
    }
    Write-Host ''
...
if ($ZscalerStatus) { Show-ZscalerStatus }  # exits 0/1 inside Show-ZscalerStatus
