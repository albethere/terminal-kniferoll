...
const (
	bannerLines  = 3 // banner + blank line + divider
	helpLines    = 2 // scroll indicator + help bar
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "a":
			// determine target: if any are unchecked, check all; else uncheck all
			anyUnchecked := false
...
func main() {
	...
	if err != nil {
		// On error, print all=true and exit
		for _, it := range m.items {
			fmt.Printf("%s=true\n", it.key)
		}
		os.Exit(1)
